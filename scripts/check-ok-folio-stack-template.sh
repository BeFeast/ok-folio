...
extension_calls="$(rg -n --glob '!**/*_test.go' 'Exec\(.*CREATE[[:space:]]+EXTENSION|Raw\(.*CREATE[[:space:]]+EXTENSION' "$repo_root/internal" "$repo_root/cmd" 2>/dev/null || true)"
if [ -n "$extension_calls" ]; then
  echo "$extension_calls" >&2
...
require_grep '--requirepass' "$compose_file" "valkey must require a password"
require_grep '--appendonly yes' "$compose_file" "valkey must enable appendonly persistence"

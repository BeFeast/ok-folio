...
func TestRecordDownloadOrDuplicateRoutesLoserToInbox(t *testing.T) {
	...
	if !bytes.Equal(exceptions[0].ContentHash, contentHash) {
		t.Fatalf("Expected duplicate inbox item to retain content hash, got %x want %x", exceptions[0].ContentHash, contentHash)
	}
	winner, err := db.GetPhotoByContentHash(contentHash)
	if err != nil {
		t.Fatalf("Expected content hash lookup to find winner: %v", err)
	}
	if winner.ID != first.ID {
		t.Fatalf("Expected content hash lookup to return first photo id %d, got %d", first.ID, winner.ID)
	}
}
